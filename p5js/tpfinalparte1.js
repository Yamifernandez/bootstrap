...
function preload() {
  // carga automática de los textos
  for (let i = 0; i <= 28; i++) {
...
function seleccionar(opcion) {
  ...
  if (estado === 4 && opcion === "A") { // DecisionG
    estado = 9; // va a Texto9 → luego secuencia 7-8
    return;
  }

  if (estado === 4 && opcion === "B") { // DecisionH
    estado = 5; // secuencia 5 → 10 → 11
    return;
  }
  ...
  if (estado === 6 && opcion === "A") { // DecisionK
    estado = 11; // activa secuencia 11 → 12
    return;
  }
  ...
  else if (estado === 1 && opcion === "A") estado = 11; // toco piedra → 11 → 12
